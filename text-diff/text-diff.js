(() => {
  ...
  function renderDiff(diff) {
    ...
    const showAdded = document.getElementById('show-added').classList.contains('active');
    const showRemoved = document.getElementById('show-removed').classList.contains('active');
    const showUnchanged = document.getElementById('show-unchanged').classList.contains('active');

    output.innerHTML = '';

    if (diff.length === 0) {
      output.innerHTML = '<div class="text-muted text-center py-4">No differences found.</div>';
      return;
    }
  ...
  function clearAll() {
    ...
    document.getElementById('diff-output').innerHTML = '<div class="text-muted text-center py-4">Click "Compare Texts" to see the differences.</div>';
  ...
  function init() {
    ...
    document.getElementById('show-added').addEventListener('click', toggleDiffType);
    document.getElementById('show-removed').addEventListener('click', toggleDiffType);
    document.getElementById('show-unchanged').addEventListener('click', toggleDiffType);
    ...
    function debounceDiff() {
      ...
      timeout = setTimeout(() => {
        if (document.getElementById('original-text').value || document.getElementById('modified-text').value) {
          performDiff();
        }
      }, 300);
    }

    document.getElementById('original-text').addEventListener('input', debounceDiff);
    document.getElementById('modified-text').addEventListener('input', debounceDiff);
  }
