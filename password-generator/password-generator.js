(() => {
  ...
  function getStrengthLevel(entropy) {
    if (entropy < 40) return { level: 0, text: 'Very Weak', color: 'bg-danger' };
    if (entropy < 60) return { level: 25, text: 'Weak', color: 'bg-warning' };
    if (entropy < 80) return { level: 50, text: 'Fair', color: 'bg-info' };
    if (entropy < 100) return { level: 75, text: 'Strong', color: 'bg-primary' };
  ...
  function clearPassword() {
    ...
    document.getElementById('strength-text').textContent = 'Select options and generate a password';
  }
  ...
  function init() {
    ...
    document.getElementById('regenerate-btn').addEventListener('click', generateAndDisplay);
