(() => {
  ...
  async function analyzeVulnerabilities() {
    ...
    if (!jsonInput) {
      showError(
        "Please paste your package-lock.json content or upload a file.",
      );
      return;
    }
    ...
    try {
      ...
      showError(
        "Invalid JSON. Please check the format of your package-lock.json.",
      );
      return;
    }
    ...
    document.getElementById("vulnerable-packages").textContent =
    ...
    const enrichedResults =
      await fetchAllVulnerabilityDetails(vulnerablePackages);
    ...
    for (const { name, version, vulns } of enrichedResults) {
      for (const vuln of vulns) {
        ...
        const severity =
          details.database_specific?.severity ||
          severityObj.score ||
          severityObj.type ||
          vuln.severity ||
          "UNKNOWN";
        ...
        const published =
          details.published || vuln.published
            ? new Date(details.published || vuln.published).toLocaleDateString()
            : "Unknown";
        const modified = details.modified
          ? new Date(details.modified).toLocaleDateString()
          : "Unknown";
        const summary =
          details.summary || vuln.summary || "No summary available";
        const description = details.description || "";
        const affected = details.affected || [];

        const githubAdvisoryUrlPattern =
          /^https:\/\/github\.com\/[^/]+\/[^/]+\/security\/advisories\//;
        const references = (details.references || vuln.references || []).filter(
          (ref) =>
            ref.type === "ADVISORY" ||
            (ref.type === "WEB" && githubAdvisoryUrlPattern.test(ref.url)),
        );
                    ...
                    .map((r) => {
                      if (r.type === "ECOSYSTEM" && r.events) {
                        const events = r.events
                          .map((e) => {
                            if (e.introduced)
                              return `introduced: ${e.introduced}`;
        ...
        const githubReviewStatus =
          details.database_specific?.github_review_status || "";
  ...
  function init() {
    document
      ...
      .addEventListener("click", analyzeVulnerabilities);
