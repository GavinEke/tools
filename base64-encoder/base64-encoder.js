(() => {
  ...
  async function encodeInput() {
    ...
    const activeTab = document.querySelector(
      "#encode-input-tabs .nav-link.active",
    ).id;
    ...
    if (encoded) {
      ...
      document.getElementById("encode-stats").textContent =
        `Encoded ${inputType} (${encoded.length} characters)`;
      document.getElementById("encode-stats").classList.remove("d-none");
      document
        .getElementById("encode-output-container")
  ...
  function decodeInput() {
    ...
    if (!base64Input) {
      ...
      document
        .getElementById("decode-output-container")
        .classList.add("text-body-secondary");
    ...
    try {
      ...
      if (isText) {
        ...
        document.getElementById("decode-stats").textContent =
          `Decoded text (${decoded.length} characters)`;
        document.getElementById("decode-download-btn").classList.add("d-none");
      } else {
        document.getElementById("decode-output").textContent =
          `Binary data detected (${decoded.length} bytes). Use download button.`;
        document.getElementById("decode-stats").textContent =
          `Decoded binary (${decoded.length} bytes)`;
        document
          .getElementById("decode-download-btn")
  ...
  async function copyDecodeToClipboard() {
    ...
    if (
      output !== "Decoded output will appear here" &&
  ...
  function init() {
    ...
    document
      ...
      .addEventListener("click", encodeInput);
    document
      .getElementById("encode-clear-btn")
      .addEventListener("click", clearEncode);
    document
      .getElementById("encode-copy-btn")
      .addEventListener("click", copyEncodeToClipboard);
    document
      .getElementById("encode-download-btn")
      .addEventListener("click", downloadEncodeOutput);

    document
      .getElementById("encode-text-input")
    ...
    document
      .getElementById("encode-file-input")
    ...
    document
      ...
      .addEventListener("click", decodeInput);
    document
      .getElementById("decode-clear-btn")
      .addEventListener("click", clearDecode);
    document
      .getElementById("decode-copy-btn")
      .addEventListener("click", copyDecodeToClipboard);
    document
      .getElementById("decode-download-btn")
      .addEventListener("click", downloadDecodeOutput);
