(() => {
  ...
  function parseDmarcXml(xmlText) {
    ...
    if (metadata) {
      report.metadata = {
        ...
        extra_contact_info:
          metadata.querySelector("extra_contact_info")?.textContent || "",
    ...
    report.records = Array.from(records).map((record) => {
      ...
      return {
        row: {
          ...
          policy_evaluated: {
            disposition:
              row?.querySelector("policy_evaluated disposition")?.textContent ||
              "",
            dkim:
              row?.querySelector("policy_evaluated dkim")?.textContent || "",
            spf: row?.querySelector("policy_evaluated spf")?.textContent || "",
          },
        },
        identifiers: {
          header_from:
            identifiers?.querySelector("header_from")?.textContent || "",
          envelope_from:
            identifiers?.querySelector("envelope_from")?.textContent || "",
          envelope_to:
            identifiers?.querySelector("envelope_to")?.textContent || "",
        },
        auth_results: {
          dkim: Array.from(auth_results?.querySelectorAll("dkim") || []).map(
            (dkim) => ({
          ...
          spf: Array.from(auth_results?.querySelectorAll("spf") || []).map(
            (spf) => ({
  ...
  function renderResults(report) {
    ...
    if (report.records && report.records.length > 0) {
      html += "<h6>Records</h6>";
      html +=
        '<div class="table-responsive"><table class="table table-striped table-sm"><thead><tr>';
      html +=
        "<th>Source IP</th><th>Count</th><th>Disposition</th><th>DKIM</th><th>SPF</th><th>Header From</th>";
      html += "</tr></thead><tbody>";

      report.records.forEach((rec) => {
        const dkimResults = rec.auth_results.dkim
          .map((d) => `${d.result} (${d.domain})`)
          .join(", ");
        const spfResults = rec.auth_results.spf
          .map((s) => `${s.result} (${s.domain})`)
          .join(", ");
