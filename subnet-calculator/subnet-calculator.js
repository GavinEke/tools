(() => {
  ...
  function calculateSubnet() {
    ...
    document.getElementById('result-hosts').textContent = formatNumber(Math.pow(2, 32 - cidr));
    document.getElementById('result-usable').textContent = formatNumber(usableHosts);
    document.getElementById('result-class').textContent = getIpClass(ipInput);
    document.getElementById('result-type').textContent = getAddressType(ipInput, networkIp, broadcastIp);

    document.getElementById('result-binary-ip').textContent = `IP:      ${longToBinaryString(ipLong)}`;
    document.getElementById('result-binary-mask').textContent = `Mask:    ${longToBinaryString(mask)}`;
    document.getElementById('result-binary-network').textContent = `Network: ${longToBinaryString(networkLong)}`;
