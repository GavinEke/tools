    ...
    for (const { name, version, vulns } of enrichedResults) {
      for (const vuln of vulns) {
        ...
        const githubReviewStatus =
          ...
          vuln.database_specific?.github_review_status;
        ...
        const filteredReferences = references.filter(
          (ref) =>
            (ref.type === "ADVISORY" || ref.type === "WEB") &&
            githubAdvisoryUrlPattern.test(ref.url),
        );
