(function () {
  ...
  function analyzeHeaders() {
    ...
    // Categorize headers
    const basicHeaders = ['From', 'To', 'Cc', 'Bcc', 'Subject', 'Date', 'Message-ID'];
    const authHeaders = ['Authentication-Results', 'DKIM-Signature', 'Received-SPF', 'ARC-Authentication-Results'];
    ...
    const others = headers.filter(h => !basicHeaders.includes(h.key) && !authHeaders.includes(h.key) && h.key.toLowerCase() !== 'received');
    ...
    if (receivedHeaders.length > 0) {
      ...
      html += '<ol class="mb-4">';
      receivedHeaders.reverse().forEach(header => {  // Reverse to show chronological order
    ...
    html += '<div class="table-responsive"><table class="table table-striped table-sm"><thead><tr><th>Header</th><th>Value</th></tr></thead><tbody>';
