(() => {
  ...
  function displayResults(data) {
    ...
    document.getElementById('result-version').textContent = data.version || 'N/A';
    document.getElementById('result-hostname').textContent = data.hostname || 'N/A';
    document.getElementById('result-asn').textContent = data.asn ? `AS${data.asn}` : 'N/A';
    document.getElementById('result-isp').textContent = data.org || data.isp || 'N/A';
    document.getElementById('result-org').textContent = data.org || 'N/A';

    document.getElementById('result-country').textContent = data.country_name ? `${getCountryFlag(data.country_code)} ${data.country_name}` : 'N/A';
    ...
    document.getElementById('result-utc-offset').textContent = data.utc_offset || 'N/A';

    if (data.latitude && data.longitude) {
      document.getElementById('result-coords').textContent = `${data.latitude}, ${data.longitude}`;
      updateMap(data.latitude, data.longitude, data.city || data.region || data.country_name || 'Unknown');
    } else {
  ...
  async function handleLookup() {
    ...
    try {
      ...
      hideLoading();
      showError(error.message || 'Failed to look up IP address. Please try again.');
    }
  }
