(() => {
    ...
    function parseCron(expression) {
        ...
        nextRunsBody.innerHTML = nextRuns.map(time => `<li>${time}</li>`).join('');
    }
    ...
    function explainCron(fields) {
        ...
        const fieldDetails = [
            ...
            { name: 'Month', values: expandField(month, 1, 12).map(m => ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][m - 1]) },
            { name: 'Day of Week', values: expandField(dayOfWeek, 0, 7).map(d => d === 7 ? 0 : d).map(d => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]) }
        ];

        return fieldDetails.map(({ name, values }) => `<strong>${name}:</strong> ${values.join(', ')}`).join('<br>');
    }
    ...
    function getNextRuns(fields, count) {
        ...
        const dayOfWeekSet = new Set(expandField(dayOfWeek, 0, 7).map(d => d === 7 ? 0 : d));
        ...
        for (let i = 0; i < count; i++) {
            current = findNext(current, minuteSet, hourSet, dayOfMonthSet, monthSet, dayOfWeekSet);
    ...
    function findNext(start, minuteSet, hourSet, dayOfMonthSet, monthSet, dayOfWeekSet) {
