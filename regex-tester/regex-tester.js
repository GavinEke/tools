(() => {
  ...
  function getRegexFlags() {
    ...
    if (document.getElementById("flag-case-insensitive").checked)
      flags.push("i");
  ...
  function testRegex() {
    ...
    if (!pattern) {
      resultsElement.innerHTML =
        '<div class="text-body-secondary">Enter a regex pattern to begin testing</div>';
      return;
    }

    if (!testString) {
      resultsElement.innerHTML =
        '<div class="text-body-secondary">Enter text to test against the regex pattern</div>';
      return;
    }
    ...
    try {
      ...
      resultsElement.innerHTML =
        '<div class="text-body-secondary">Fix the regex pattern to see results</div>';
    }
  }
  ...
  function init() {
    ...
    document
      ...
      .addEventListener("input", debounceTestRegex);
    document
      .getElementById("test-string")
      .addEventListener("input", debounceTestRegex);
    ...
    document.getElementById("test-string").value =
      "Contact me at john@example.com or support@company.org for help.";
    testRegex();
  }
